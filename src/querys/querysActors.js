...
const createActor = async (first_name, last_name) => {
    const [query] = await connection.execute(`INSERT INTO sakila.actor (first_name,
    last_name) VALUES (?, ?)`, [first_name, last_name]);
...
const deleteActor = async (id) => {
    ...
    sakila.film_actor WHERE actor_id = ?;`, [id]);
    const [query] = await connection
    .execute(`DELETE FROM sakila.actor 
    WHERE actor_id = ?;`, [id]);
    return query;
}
